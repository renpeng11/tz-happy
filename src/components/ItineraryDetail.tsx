...
export default function ItineraryDetail() {
    ...
    try {
      ...
      const daysExpenses = route.days.map(day => {
        ...
        return {
          ...
          total: dayExpenses.reduce((sum: number, exp: any) => sum + exp.amount, 0),
        };
      });
      ...
      if (route) {
        setExpenses({
          routeId,
          days: route.days.map(day => ({ day: day.day, expenses: [], total: 0 })),
          total: 0,
        });
      }
    }
        ...
        setExpenses(prev => {
          ...
          const newDays = prev.days.map(d => {
            if (d.day === day) {
              ...
              return { ...d, expenses: newExpenses, total: newExpenses.reduce((sum, e) => sum + e.amount, 0) };
            }
            return d;
          });
          return { ...prev, days: newDays, total: newDays.reduce((sum, d) => sum + d.total, 0) };
        });
      }
    } catch (error) {
  ...
  const ExpenseList = ({ day, expenses }: { day: number; expenses: ExpenseItem[] }) => {
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm border border-sky-100">
        ...
        <div className="space-y-2">
          {expenses.map(expense => (
            <div key={expense.id} className="flex items-center justify-between p-2 bg-sky-50 rounded-lg">
              <div className="flex-1">
                <div className="text-sm font-medium text-text">{expense.name}</div>
                <div className="text-xs text-textLight">{expense.category} · {expense.time}</div>
              </div>
              <div className="flex items-center gap-2">
                <div className="text-sm font-bold text-primary">¥{expense.amount.toFixed(2)}</div>
                <button
                  onClick={() => deleteExpense(day, expense.id)}
        ...
        <div className="mt-3 pt-3 border-t border-sky-200 flex justify-between items-center">
          ...
          <span className="text-lg font-bold text-primary">¥{expenses.reduce((sum, e) => sum + e.amount, 0).toFixed(2)}</span>
        </div>
      </div>
    );
  };
        ...
        <div className={showAllDays ? 'lg:col-span-2' : 'lg:col-span-2'}>
          <div className="space-y-6">
            {winnerRoute.days.map((day) => {
              ...
              return (
                <div key={day.day} className="bg-white rounded-[20px] p-6 shadow-lg border border-sky-100">
                    ...
                    <div className="mb-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {day.spots.map((spotName) => {
                          ...
                          return <SpotCardComponent key={spotName} spotName={spotName} />;
                        })}
                      </div>
                    </div>
                  )}
                  ...
                  <ExpenseList day={day.day} expenses={expenses.days.find(d => d.day === day.day)?.expenses || []} />
                </div>
              );
            })}
          </div>
